...
func (h *Handler) registerRoutes() error {
	routes := []route{
		...
		{RouteSpec: RouteSpec{http.MethodGet, "/api/v1/teams/{teamId}/overview/sessions", WebOnly}, handler: h.teamOverviewSessions, body: noRequestBody, cache: noStore, actions: []authorization.Action{authorization.TeamReadMetadata}},
		...
		{RouteSpec: RouteSpec{http.MethodPost, "/api/v1/ingestion/raw/receipts/lookup", CLIOnly}, handler: h.rawReceipt, body: controlPlaneJSONRequest, cache: noStore, actions: []authorization.Action{authorization.RawIngest}},
