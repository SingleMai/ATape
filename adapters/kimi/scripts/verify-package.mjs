...
const root = await mkdtemp(join(tmpdir(), "atape-kimi-package-"))
...
try {
  ...
  const entryRoot = join(installed, "node_modules", "@atape", "adapter-kimi")
  ...
  assert.equal(manifest.name, "@atape/adapter-kimi")
  ...
  assert.equal(manifest.atapeAdapter.adapterId, "kimi")
  ...
  for (const name of ["native", "context", "auto", "clear"].flatMap(name => [`${name}-0.42.0.jsonl`, `${name}-0.42.0.state.json`])) {
