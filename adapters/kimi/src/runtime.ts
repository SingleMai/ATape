...
import { kimiHome } from "@atape/adapter-catalog/node"
...
export const createAtapeAdapter = async (context: AdapterOpenContext & { readonly signal: AbortSignal }): Promise<SourceAdapterRuntime> => {
  const home = kimiHome(process.env, homedir()), lifetime = new AbortController()
  ...
  const run = <A>(f: (signal: AbortSignal) => Promise<A>, signal: AbortSignal) => Effect.runPromise(Effect.tryPromise({
    try: async signal => { if (stopped) fail("closed", "Kimi runtime is closed."); return f(signal) }, catch: sourceError
  ...
  return { close, sourceCapture: { protocolVersion: SourceCaptureVersion,
    ...
    open: request => run(async signal => {
      if (active) fail("closed", "Kimi runtime already has an open view.")
      active = true
      try {
        ...
        let frames: ReadonlyArray<SourceCaptureFrame> = planned.frames, at = 0, disposed = false, exhausted = false
        ...
        return { ...planned.header,
          read: (signal: AbortSignal) => run(async signal => {
            if (disposed || exhausted) fail("closed", "Kimi view is closed.")
            if (performance.now() - started > limits.durationMs) fail("limit", "Kimi view exceeded its deadline.")
            ...
            exhausted = at === frames.length
            return { frames: page, done: exhausted }
          }, signal),
          close: async () => { dispose() }
        }
