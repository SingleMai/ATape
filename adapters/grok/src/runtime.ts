...
import { grokHome } from "@atape/adapter-catalog/node"
...
export const createAtapeAdapter = async (context: AdapterOpenContext & { readonly signal: AbortSignal }): Promise<SourceAdapterRuntime> => {
  const home = grokHome(process.env, homedir()), lifetime = new AbortController()
  ...
  const run = <A>(f: (signal: AbortSignal) => Promise<A>, signal: AbortSignal) => Effect.runPromise(Effect.tryPromise({
    try: async signal => { if (stopped) fail("closed", "Grok runtime is closed."); return f(signal) }, catch: sourceError
  ...
  return { close, sourceCapture: { protocolVersion: SourceCaptureVersion,
    ...
    open: request => run(async signal => {
      if (active) fail("closed", "Grok runtime already has an open view.")
      active = true
      try {
        ...
        return { ...planned.header,
          read: (signal: AbortSignal) => run(async signal => {
            if (disposed) fail("closed", "Grok view is closed.")
            if (performance.now() - started > limits.durationMs) fail("limit", "Grok view exceeded its deadline.")
