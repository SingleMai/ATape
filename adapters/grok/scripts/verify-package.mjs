...
const root = await mkdtemp(join(tmpdir(), "atape-grok-package-"))
...
try {
  ...
  const entryRoot = join(installed, "node_modules", "@atape", "adapter-grok")
  ...
  assert.equal(manifest.name, "@atape/adapter-grok")
  ...
  assert.equal(manifest.atapeAdapter.adapterId, "grok")
