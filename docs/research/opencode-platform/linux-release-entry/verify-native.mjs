...
const packageManifest=JSON.parse(await readFile(join(dirname(dirname(process.argv[2])),'package.json'),'utf8'));
assert.equal(packageManifest.name,'@atape/adapter-opencode');assert.equal(packageManifest.version,'0.4.8');
assert.equal(packageManifest.atapeAdapter.protocolVersion,'atape.adapter.v1alpha1');
assert.equal(packageManifest.atapeAdapter.sourceCapture,'atape.source-capture.v1');
assert.equal(packageManifest.atapeAdapter.gitAttribution,'atape.git-attribution.v1');
...
const runtime=await createAtapeAdapter({protocolVersion:'atape.adapter.v1alpha1',adapter:{id:packageManifest.atapeAdapter.adapterId,version:packageManifest.version},project:{id:'controlled',type:'directory',path:manifest.projectPath},signal:lifetime.signal});
...
const result={installedPackage:{name:packageManifest.name,version:packageManifest.version,atapeAdapter:packageManifest.atapeAdapter},platform:process.platform,arch:process.arch,node:process.version,artifactSHA256:createHash('sha256').update(await readFile(process.argv[2])).digest('hex'),discovery:[],captures:[]};
try{
