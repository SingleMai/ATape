...
const runtime=await createAtapeAdapter({protocolVersion:'atape.adapter.v1alpha1',adapter:{id:'opencode',version:'0.0.0'},project:{id:'controlled',type:'directory',path:manifest.projectPath},signal:lifetime.signal});
...
const result={platform:process.platform,arch:process.arch,node:process.version,artifactSHA256:createHash('sha256').update(await readFile(process.argv[2])).digest('hex'),discovery:[],captures:[]};
try{
