...
| [adapters](../adapters/) | Codex, Claude, OpenCode, CodeBuddy and Grok provider Implementations; the Host loads their declared runtime capability |
...
| Collector / Server delivery | `pnpm test:e2e` for Codex/Claude; `pnpm test:opencode-contract`, `pnpm test:codebuddy-contract` and `pnpm test:grok-contract` for installed source-capture Adapters over authenticated HTTP/PostgreSQL |
...
OpenCode, CodeBuddy and Grok contracts; its name is narrower than its coverage.
