...
| [adapters](../adapters/) | Codex, Claude, OpenCode, CodeBuddy and Kimi provider Implementations; the Host loads their declared runtime capability |
...
| Collector / Server delivery | `pnpm test:e2e` for Codex/Claude; `pnpm test:opencode-contract`, `pnpm test:codebuddy-contract` and `pnpm test:kimi-contract` for installed source-capture Adapters over authenticated HTTP/PostgreSQL |
...
OpenCode, CodeBuddy and Kimi contracts; its name is narrower than its coverage.
