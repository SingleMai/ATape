...
export const requiredTest = "TestHTTPAuthenticationAndAuthorizationContract/native_Grok_Collector"
export function verifyGrokResult(events) {
  if (!events.some(event => event.Action === "pass" && event.Test === requiredTest &&
    ...
    throw new Error(`Required Grok contract did not pass (missing or skipped): ${requiredTest}`)
  }
}

async function run() {
  const args = process.argv.slice(2)
  if (args.some(arg => arg !== "--all") || args.length > 1) throw new Error("Use verify-grok-contract.mjs [--all]")
  const packages = args.includes("--all")
    ...
    : ["./internal/adapters/httpapi", "-run", "^TestHTTPAuthenticationAndAuthorizationContract$/^native_Grok_Collector$"]
  ...
  try {
    ...
    console.log("Required Grok/PostgreSQL/installed-CLI contract passed.")
  } finally {
    lines.close()
