...
export const requiredTest = "TestHTTPAuthenticationAndAuthorizationContract/native_Kimi_Collector"
export function verifyKimiResult(events) {
  if (!events.some(event => event.Action === "pass" && event.Test === requiredTest &&
    ...
    throw new Error(`Required Kimi contract did not pass (missing or skipped): ${requiredTest}`)
  }
}

async function run() {
  const args = process.argv.slice(2)
  if (args.some(arg => arg !== "--all") || args.length > 1) throw new Error("Use verify-kimi-contract.mjs [--all]")
  const packages = args.includes("--all")
    ...
    : ["./internal/adapters/httpapi", "-run", "^TestHTTPAuthenticationAndAuthorizationContract$/^native_Kimi_Collector$"]
  ...
  try {
    ...
    console.log("Required Kimi/PostgreSQL/installed-CLI contract passed.")
  } finally {
    lines.close()
