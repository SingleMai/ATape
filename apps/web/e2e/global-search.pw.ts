...
for (const width of [390, 1440]) {
  test(`keeps the account avatar reachable with the sidebar collapsed at ${width}px`, async ({ page }) => {
    ...
    await expect(page.getByRole("region", { name: "atape-cli" })).toBeVisible()
...
test("settings preserves the reader and supports nested confirmation without dismissing settings", async ({ page }) => {
  ...
  await expect(settings.getByRole("combobox", { name: "Personal Raw upload preference" })).toBeVisible()
  await settings.getByRole("button", { name: "Close settings" }).focus()
  await page.keyboard.press("Shift+Tab")
  await expect(settings.getByRole("combobox", { name: "Personal Raw upload preference" })).toBeFocused()
  await page.keyboard.press("Tab")
  await expect(settings.getByRole("button", { name: "Close settings" })).toBeFocused()
  await settings.getByRole("button", { name: "atape-cli", exact: true }).click()
  await settings.locator(".cli-device-details > summary").click()
  await settings.getByRole("button", { name: "Disconnect device", exact: true }).click()
...
test("settings does not expose or request browser session management", async ({ page }) => {
  ...
  await settings.getByRole("button", { name: "atape-cli", exact: true }).click()
  await expect(settings.getByRole("heading", { name: "atape-cli", level: 1 })).toBeVisible()
