...
test("keeps each conversation turn focused on the prompt and final response", async ({ context, page }) => {
  ...
  await expect(page.getByText("Tool activity").first()).toBeVisible()
  await expect(page.locator(".process-tool").first().getByText("exec · completed")).not.toBeVisible()
  await page.locator(".process-tool-group > summary").first().click()
  ...
  await expect(page.locator(".process-tool-group").first()).toHaveAttribute("open", "")
