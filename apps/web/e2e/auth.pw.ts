...
test("persists Raw preference and Team policy with the effective state", async ({ context, page }) => {
  await authenticate(context)
  await page.goto("/settings/account")
  const preference = page.getByRole("combobox", { name: "Personal Raw upload preference" })
  await expect(preference).toHaveValue("disable")
  await preference.selectOption("enable")
  await expect(page.getByRole("status")).toContainText("Saved.")
  expect((await fixtureState(page)).rawPreference).toBe("enable")
  await page.goto("/teams/team-a/settings/access")
  const policy = page.getByRole("combobox", { name: "Team Raw upload policy" })
  await expect(policy).toHaveValue("personal")
  await expect(page.getByText("Raw upload for you: Enabled")).toBeVisible()
  await policy.selectOption("close")
  await expect(page.getByText("Raw upload for you: Disabled")).toBeVisible()
