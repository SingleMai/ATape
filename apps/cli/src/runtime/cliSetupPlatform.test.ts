...
const fixture = async (override: (root: string) => Record<string, string> = () => ({})) => {
  ...
  roots.push(root)
  const environment = { ATAPE_HOME: root, ATAPE_CODEX_HOME: join(root, "codex"), ATAPE_CLAUDE_HOME: join(root, "missing-claude"), ATAPE_CODEBUDDY_HOME: join(root, "missing-codebuddy"), ATAPE_KIMI_HOME: join(root, "missing-kimi"),
