...
describe("Source record journal", () => {
  ...
  it("fences metadata access and refuses canceled or changed Raw reuse", async () => {
    ...
    await f.run(Effect.gen(function*() {
      ...
      expect((yield* j.recordStatus(first, "original", rawKey).pipe(Effect.flip)).reason).toBe("conflict")
      expect(yield* j.recordStatus(owner, "original", rawKey)).toEqual((yield* j.records(owner, "original", { kind: "raw" }))[0])
      expect(yield* j.recordStatus(owner, "original", { kind: "raw", key: "missing" })).toBeNull()
      ...
      expect((yield* j.recordStatus(owner, "original", rawKey))!.disposition).toBe("canceled")
