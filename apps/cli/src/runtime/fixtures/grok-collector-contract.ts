...
if (input.phase === "edit") writeNative("shell")
if (["edit", "raw-off", "lose-activation", "raw-only"].includes(input.phase)) {
  ...
  if (input.phase === "edit") rows[12].params.update.content.text += " GrokResumeNeedle SENSITIVE_TEST_TOKEN"
  if (input.phase === "raw-off") last.params.update.content.text = "GrokPolicyNeedle"
  if (input.phase === "lose-activation") last.params.update.content.text = "GrokFinalNeedle"
  if (input.phase === "raw-only") last.extraRawField = "GrokRawOnlyNeedle"
  writeFileSync(file, rows.map(row => JSON.stringify(row) + "\n").join(""))
}
if (["recover-activation", "recover-raw"].includes(input.phase)) rmSync(directory, { recursive: true })
if (["restore", "repair"].includes(input.phase)) {
  writeNative("shell"); writeFileSync(file, readFileSync(join(home, "saved.jsonl")))
}
...
const faultFetch: typeof fetch = async (url, init) => {
  ...
  if (!lost && (["lose-activation"].includes(input.phase) && target.endsWith("/activate") && response.status === 200 || ["raw-only"].includes(input.phase) && target.endsWith("/ingestion/raw/chunks") && response.status === 201)) {
...
const result = await Effect.runPromise(Effect.gen(function*() {
  ...
  if (["initial", "upgrade", "edit", "git-initial"].includes(input.phase)) {
    ...
    for (let cycle = 0; cycle < 5; cycle++) {
      ...
      if (["malformed"].includes(input.phase)) { assert.ok(diagnostics > 0); break }
