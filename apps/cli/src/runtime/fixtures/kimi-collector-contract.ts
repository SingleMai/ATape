...
const specimen = input.phase.startsWith("background-") ? "background" : input.phase.startsWith("nested-subagents-") ? "nested-subagents" : input.phase.startsWith("subagents-") ? "subagents" : input.phase === "nested-fork" ? "nested-fork" : input.phase.startsWith("fork-") ? "fork" : input.phase.startsWith("context-") ? "context" : input.phase === "auto" ? "auto" : input.phase === "clear" ? "clear" : "native"
const home = input.home, workspace = join(home, "workspace"), adapterId = "kimi"
const native = readFileSync(new URL((["subagents", "nested-subagents", "background"].includes(specimen) ? `../../../../../adapters/kimi/src/fixtures/${specimen}-0.42.0/agents/main/wire.jsonl` : `../../../../../adapters/kimi/src/fixtures/${specimen}-0.42.0.jsonl`), import.meta.url), "utf8").replaceAll("/fixture/kimi-project", workspace)
const metadata = readFileSync(new URL((["subagents", "nested-subagents", "background"].includes(specimen) ? `../../../../../adapters/kimi/src/fixtures/${specimen}-0.42.0/state.json` : `../../../../../adapters/kimi/src/fixtures/${specimen}-0.42.0.state.json`), import.meta.url), "utf8").replaceAll("/fixture/kimi-project", workspace)
...
const backgroundLength: Record<string, number> = { "background-seed": 38, "background-resume": 69, "background-restore": 69, "background-foreground": 88, "background-final": 119 }
if (backgroundLength[input.phase]) {
  const length = backgroundLength[input.phase]!, meta = JSON.parse(metadata)
  if (length < 119) delete meta.agents["agent-1"]
  restore(native.split("\n").slice(0, length).join("\n") + "\n"); writeFileSync(join(directory, "state.json"), JSON.stringify(meta))
  for (const child of length < 119 ? ["agent-0"] : ["agent-0", "agent-1"]) {
    const path = join(directory, "agents", child, "wire.jsonl")
    const rows = readFileSync(new URL(`../../../../../adapters/kimi/src/fixtures/background-0.42.0/agents/${child}/wire.jsonl`, import.meta.url), "utf8").replaceAll("/fixture/kimi-project", workspace).trim().split("\n")
    mkdirSync(dirname(path), { recursive: true }); writeFileSync(path, rows.slice(0, length === 38 ? 25 : length === 69 ? 37 : rows.length).join("\n") + "\n")
  }
}
if (["background-recover", "background-recover-raw"].includes(input.phase)) rmSync(directory, { recursive: true })
if (input.phase === "background-running") {
  const rows = native.trim().split("\n").map(line => JSON.parse(line))
  save(rows.slice(0, 108)) // Second task launched, but its terminal record and notification have not arrived.
}
if (input.phase === "background-raw-loss") {
  const rows = native.trim().split("\n").map(line => JSON.parse(line))
  rows[108].extraRawField = "KimiBackgroundRawOnly"; save(rows)
}
...
const faultFetch: typeof fetch = async (url, init) => {
  ...
  if (!lost && (["lose-activation", "context-undo", "fork-undo", "subagents-resume", "nested-subagents-resume", "background-resume"].includes(input.phase) && target.endsWith("/activate") && response.status === 200 || ["raw-only", "context-raw-loss", "subagents-raw-loss", "nested-subagents-raw-loss", "background-raw-loss"].includes(input.phase) && target.endsWith("/ingestion/raw/chunks") && response.status === 201)) {
...
const result = await Effect.runPromise(Effect.gen(function*() {
  ...
  if (["initial", "upgrade"].includes(input.phase)) {
    ...
    for (let cycle = 0; cycle < 5; cycle++) {
      ...
      if (["malformed", "unsupported", "context-incomplete", "subagents-incomplete", "nested-subagents-incomplete", "background-running"].includes(input.phase)) { assert.ok(diagnostics > 0); break }
  ...
  if (["context-undo", "context-raw-loss", "fork-undo", "subagents-resume", "subagents-raw-loss", "nested-subagents-resume", "nested-subagents-raw-loss", "background-resume", "background-raw-loss"].includes(input.phase)) assert.equal(lost, true)
  if (["context-recover-raw", "subagents-recover-raw", "nested-subagents-recover-raw", "background-recover-raw"].includes(input.phase)) assert.equal(uploads, 0)
  if (["context-raw-on", "fork-raw-on", "subagents-raw-on", "nested-subagents-raw-on", "background-raw-on"].includes(input.phase)) { assert.equal(puts, 0); assert.ok(uploads > 0) }
