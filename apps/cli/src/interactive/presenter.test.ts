...
const fixture = async (setup = false, update?: Promise<string>, failInstall = false, failFirstResume = false, toolUpdate = false, maintenance = false) => {
  ...
  const environment = {
    ...
    ATAPE_CODEX_HOME: join(root, "no-codex"), ATAPE_CLAUDE_HOME: join(root, "no-claude"), ATAPE_CODEBUDDY_HOME: join(root, "no-codebuddy"), ATAPE_KIMI_HOME: join(root, "no-kimi"), OPENCODE_DB: join(root, "no-opencode.db")
  }
...
describe("interactive navigation through the presenter Interface", () => {
  ...
  it("opens global checkboxes from Tools, cancels without saving and keeps project recovery direct", async () => {
    ...
    expect(tools.options).toEqual([{ value: "codex", label: "Codex" }, { value: "claude", label: "Claude Code" }, { value: "codebuddy", label: "CodeBuddy Code CLI" }, { value: "kimi", label: "Kimi Code CLI" }, { value: "opencode", label: "OpenCode" }])
